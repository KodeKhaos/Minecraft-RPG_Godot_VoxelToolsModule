...
func generate(seed: int) -> Structure:
	...
	# Trunk
	var trunk_len := int(random.randf_range(trunk_len_min, trunk_len_max))
	for y in trunk_len:
		voxels[Vector3(0, y, 0)] = log_type

	# Branches
	var branches_start := int(random.randf_range(trunk_len / 3, trunk_len / 2))
	for y in range(branches_start, trunk_len):
		var t := float(y - branches_start) / float(trunk_len)
		var branch_chance := 1.0 - pow(t - 0.5, 2)
		if random.randf() < branch_chance:
			var branch_len := int((trunk_len / 2.0) * branch_chance * random.randf())
