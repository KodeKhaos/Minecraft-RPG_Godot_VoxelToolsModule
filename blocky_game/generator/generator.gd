...
func _init():
	...
	for i in 16:
		var s = tree_generator.generate(global_seed)
	...
	_heightmap_noise.seed = global_seed
...
func _generate_block(buffer: VoxelBuffer, origin_in_voxels: Vector3i, lod: int):
	...
	if origin_in_voxels.y > _heightmap_max_y:
		...
		rng.seed = _get_chunk_seed_2d(chunk_pos, global_seed)
...
func _get_tree_instances_in_chunk(
	...
	var rng := RandomNumberGenerator.new()
	rng.seed = _get_chunk_seed_2d(cpos, global_seed)
...
static func _get_chunk_seed_2d(cpos: Vector3, global_seed: int) -> int:
	return (int(cpos.x) ^ (31 * int(cpos.z))) + global_seed
